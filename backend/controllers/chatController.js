...
// ── Shared Turn Preparation (used by both blocking and streaming endpoints) ────

/**
 * Gathers all context (memories, videos, actions, past-self) for one chat turn
 * and returns everything needed to call Gemini, without actually calling it.
 * Keeping this shared means the streaming and non-streaming endpoints can never drift apart.
 */
async function prepareChatTurn(content, userId, chatId) {
  ...
  const [
    ...
    embedding
      ? findSimilarEntries(JournalEntry, { embedding, userId, limit: 5 })
          .then(results => results.filter(r => r.score > 0.4).map(r => r.entry))
          .catch(() => JournalEntry.find({ user: userId }).sort({ date: -1 }).limit(5).lean())
      : JournalEntry.find({ user: userId }).sort({ date: -1 }).limit(5).lean(),

    JournalEntry.find({ user: userId }).sort({ date: -1 }).limit(5).lean(),

    embedding
      ? findSimilarVideos(VideoReflection, { embedding, userId, limit: 3 })
          .then(results => results.filter(r => r.score > 0.4).map(r => r.video))
          .catch(() => [])
      : [],

    VideoReflection.find({ user: userId })
      .sort({ createdAt: -1 })
      .limit(10)
      .select('title note createdAt')
      .lean(),

    ActionMemory.find({ user: userId })
      .sort({ createdAt: -1 })
      .limit(30)
      .lean(),

    retrievePastSelfRecommendation(content.trim(), userId),
  ]);

  extractAndSaveActions(content.trim(), 'CompanionConversation', chatId, userId);
  ...
  return { systemPrompt, relevantVideos, pastSelfRec, riskFlag };
}
...
async function sendMessage(req, res, next) {
  try {
    ...
    const { systemPrompt, relevantVideos, pastSelfRec, riskFlag } = await prepareChatTurn(content.trim(), req.userId, chat._id);
...
// Same behavior as sendMessage, but streams the reply as Server-Sent Events so the
// companion can render tokens as they arrive instead of waiting for the full response.
async function sendMessageStream(req, res, next) {
  try {
    ...
    if (!apiKey) {
      ...
      send('done', { messages: chat.messages });
      return res.end();
    }
    ...
    try {
      ...
      console.error('[companion/stream] Gemini streaming error:', streamErr.message);
      if (!fullReply) fullReply = "I'm having trouble finding words right now — could you try that again?";
      send('chunk', { text: fullReply });
    }

    if (!fullReply.trim()) fullReply = "I'm here. What's on your mind?";
    ...
    // If headers are already sent (SSE started), we can't call next() the normal way —
    // emit an error event and close the stream instead.
    if (res.headersSent) {
      try {
        res.write(`data: ${JSON.stringify({ event: 'error', message: err.message || 'Something went wrong.' })}\n\n`);
        res.end();
...
module.exports = { getHistory, sendMessage, sendMessageStream };