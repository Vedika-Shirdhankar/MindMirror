...
async function processVideoJob(job) {
  ...
  try {
    ...
    if (jobId) pushProgress(jobId, { stage: 'processing', message: 'Analyzing your video with AI...' });
    ...
    const analysis = await geminiCircuitBreaker.call(
      ...
      () => {
        ...
        return {
          ...
          summary: 'Reflection saved. AI insights could not be generated at this time.',
          themes: [],
          triggers: [],
          sentiment: 'neutral',
          mood_score: 5,
          actionsMentioned: [],
          aiGeneratedInsights: 'Your video is safely stored. AI analysis can be retried anytime.',
          dominant_emotion: '',
          secondary_emotions: [],
          emotional_patterns: [],
          recurring_thoughts: [],
    ...
    if (jobId) pushProgress(jobId, { stage: 'transcribed', message: 'Transcript generated. Building insights...' });
    ...
    if (jobId) pushProgress(jobId, { stage: 'embedding', message: 'Generating semantic embeddings...' });
    ...
    if (jobId) pushComplete(jobId, { reflectionId, message: 'Video analysis complete!' });
    ...
    if (jobId) pushError(jobId, `Analysis failed: ${err.message}`);
