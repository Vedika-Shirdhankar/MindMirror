  ...
  if (reading) {
    ...
    return (
      <div className="max-w-xl mx-auto px-6 py-8">
        <button onClick={() => setReading(null)} className="text-xs mb-6 flex items-center gap-1 opacity-50 hover:opacity-80 transition-opacity" style={{ color: 'var(--color-text)' }}>
          ← Back to letters
        </button>
        <div className="rounded-2xl p-6" style={{ background: 'rgba(127,119,221,0.06)', border: '1px solid rgba(127,119,221,0.15)' }}>
          ...
          <h2 className="text-lg font-medium mb-5" style={{ color: 'var(--color-text)' }}>{reading.title}</h2>
          ...
          {reading.triggerThemes?.length > 0 && (
            <div className="mt-5 pt-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
              <p className="text-xs mb-2" style={{ color: 'var(--color-text-faint)' }}>Surfaces when you feel:</p>
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-text">Messages Through Time</h1>
          <p className="text-xs mt-1 text-text/50">Write to yourself during calm moments. MindMirror delivers your envelope when you need it most.</p>
        </div>
      ...
      {/* Writing form */}
      {writing && (
        <div className="rounded-2xl p-5 mb-6 fade-up" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-surface-border)' }}>
          ...
          <input
            ...
            style={{ background: 'transparent', color: 'var(--color-text)', borderColor: 'rgba(255,255,255,0.1)' }}
          />
          <textarea
            value={body}
          ...
          <div className="mb-5">
            <p className="text-xs font-medium mb-2" style={{ color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Surface this letter when I feel…</p>
          ...
          <div className="flex gap-2">
            <button onClick={() => setWriting(false)} className="px-4 py-2 rounded-xl text-sm border transition-opacity hover:opacity-70" style={{ borderColor: 'rgba(255,255,255,0.1)', color: 'var(--color-text-faint)' }}>
              Cancel
            </button>
      ...
      {/* Letters list */}
      {loading ? (
        <p className="text-sm" style={{ color: 'var(--color-text-faint)' }}>Loading letters…</p>
      ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium mb-1 truncate" style={{ color: 'var(--color-text)' }}>{l.title}</p>
                      <p className="text-xs mb-2 leading-relaxed" style={{ color: 'var(--color-text-muted)' }}>
                        {l.body.slice(0, 100)}…
                      </p>
                      <p className="text-xs" style={{ color: 'var(--color-text-faint)' }}>{format(letterDate, 'MMM d, yyyy')}</p>
                    </div>
                  </div>
      ...
      {!loading && letters.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-sm" style={{ color: 'var(--color-text-faint)' }}>Write your first letter on a good day. Future you will thank you.</p>
        </div>
      )}
    </div>
  )
